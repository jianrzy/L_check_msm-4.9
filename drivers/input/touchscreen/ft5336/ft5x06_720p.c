...
static struct Upgrade_Info fts_updateinfo[] = {
    {0x55, "FT5x06", TPD_MAX_POINTS_5, AUTO_CLB_NEED, 50, 30, 0x79, 0x03, 1,
     2000},
    {0x08, "FT5606", TPD_MAX_POINTS_5, AUTO_CLB_NEED, 50, 30, 0x79, 0x06, 100,
     2000},
    {0x0a, "FT5x16", TPD_MAX_POINTS_5, AUTO_CLB_NEED, 50, 30, 0x79, 0x07, 1,
     1500},
    {0x05, "FT6208", TPD_MAX_POINTS_2, AUTO_CLB_NONEED, 60, 30, 0x79, 0x05, 10,
     2000},
    {0x06, "FT6x06", TPD_MAX_POINTS_2, AUTO_CLB_NONEED, 100, 30, 0x79, 0x08, 10,
     2000},
    {0x36, "FT6x36", TPD_MAX_POINTS_2, AUTO_CLB_NONEED, 100, 30, 0x79, 0x18, 10,
     2000},
    {0x55, "FT5x06i", TPD_MAX_POINTS_5, AUTO_CLB_NEED, 50, 30, 0x79, 0x03, 1,
     2000},
    {0x14, "FT5336", TPD_MAX_POINTS_10, AUTO_CLB_NONEED, 30, 30, 0x79, 0x11, 10,
     2000},
    {0x13, "FT3316", TPD_MAX_POINTS_5, AUTO_CLB_NONEED, 30, 30, 0x79, 0x11, 10,
     2000},
    {0x12, "FT5436i", TPD_MAX_POINTS_5, AUTO_CLB_NONEED, 30, 30, 0x79, 0x11, 10,
     2000},
    {0x11, "FT5336i", TPD_MAX_POINTS_5, AUTO_CLB_NONEED, 30, 30, 0x79, 0x11, 10,
     2000},
};
...
#define FT_STORE_TS_INFO(buf, id, name, max_tch, group_id, fw_vkey_support,    \
  ...
  snprintf(buf, FT_INFO_MAX_LEN,                                               \
           ...
           "fw_name\t\t= %s\n"                                                 \
           "fw_ver\t\t= %d.%d.%d\n",                                           \
           id, name, max_tch, FT_DRIVER_VERSION, group_id, fw_vkey_support,    \
           fw_name, fw_maj, fw_min, fw_sub_min)
...
static ssize_t ctp_color_proc_read(struct file *file, char __user *buf,
                                   size_t count, loff_t *ppos);
static ssize_t ctp_color_proc_write(struct file *filp,
                                    const char __user *userbuf, size_t count,
                                    loff_t *ppos);
...
#if CTP_PROC_INTERFACE
static int focal_i2c_Read(unsigned char *writebuf, int writelen,
...
static int ft5x06_ts_pinctrl_select(struct ft5x06_ts_data *ft5x06_data,
  ...
  pins_state =
      on ? ft5x06_data->gpio_state_active : ft5x06_data->gpio_state_suspend;
...
#else
static int ft5x06_ts_suspend(struct device *dev) { return 0; }

static int ft5x06_ts_resume(struct device *dev) { return 0; }
...
static int fb_notifier_callback(struct notifier_block *self,
  ...
  if (evdata && evdata->data && event == FB_EVENT_BLANK && ft5x06_data &&
      ft5x06_data->client) {
    blank = evdata->data;
...
static int ft5x06_fw_upgrade_start(struct i2c_client *client, const u8 *data,
                                   u32 data_len) {
  ...
  for (i = 0, j = 0; i < FT_UPGRADE_LOOP; i++) {
    ...
    if (r_buf[0] != info.upgrade_id_1 || r_buf[1] != info.upgrade_id_2) {
      dev_err(&client->dev,
              ...
              r_buf[0], r_buf[1], info.upgrade_id_1, info.upgrade_id_2);
    } else
      break;
  }
  ...
  for (i = 0; i < pkt_num; i++) {
    ...
    ft5x06_i2c_write(client, pkt_buf, FT_FW_PKT_LEN + FT_FW_PKT_META_LEN);
  ...
  if (is_5336_new_bootloader == FT_BLOADER_VERSION_LZ4 ||
    ...
    for (i = 0; i < FT_FINISHING_PKT_LEN_OLD_FW; i++) {
      ...
      else if (is_5336_new_bootloader == FT_BLOADER_VERSION_LZ4)
...
static void fts_ctpm_read_lockdown(struct i2c_client *client,
  ...
  dev_info(
      &client->dev, "Lockdown info: %02X %02X %02X %02X %02X %02X %02X %02X",
...
static int ft5x06_fw_upgrade(struct device *dev, bool force) {
  ...
  dev_info(dev, "Current firmware: %d.%d.%d", data->fw_ver[0], data->fw_ver[1],
           data->fw_ver[2]);
  dev_info(dev, "New firmware: %d.%d.%d", fw_file_maj, fw_file_min,
           fw_file_sub_min);
  ...
  FT_STORE_TS_INFO(data->ts_info, data->family_id, data->pdata->name,
                   ...
                   data->pdata->fw_name, data->fw_ver[0], data->fw_ver[1],
                   data->fw_ver[2]);
rel_fw:
  release_firmware(fw);
  return rc;
...
static int ft5x06_debug_data_get(void *_data, u64 *val) {
  ...
  if (ft5x06_debug_addr_is_valid(data->addr)) {
    ...
    if (rc < 0)
      dev_err(&data->client->dev, "FT read register 0x%x failed (%d)\n",
              data->addr, rc);
    else
      *val = reg;
  }
...
static int  __maybe_unused fts_ctpm_fw_upgrade_with_app_file(char *firmware_name) {
  ...
  if ((pbt_buf[fwsize - 8] ^ pbt_buf[fwsize - 6]) != 0xFF ||
    ...
    CTP_ERROR("checksum is %2x, %2x, %2x, %2x, %2x, %2x", pbt_buf[fwsize - 8],
              pbt_buf[fwsize - 6], pbt_buf[fwsize - 7], pbt_buf[fwsize - 5],
...
static int ft5x06_parse_dt(struct device *dev,
  ...
  pdata->reset_gpio = of_get_named_gpio_flags(np, "ftech,reset-gpio", 0,
                                              &pdata->reset_gpio_flags);
  if (pdata->reset_gpio < 0)
    return pdata->reset_gpio;

  pdata->irq_gpio =
      of_get_named_gpio_flags(np, "ftech,irq-gpio", 0, &pdata->irq_gpio_flags);
  if (pdata->irq_gpio < 0)
  ...
  pdata->fw_vkey_support = of_property_read_bool(np, "ftech,fw-vkey-support");

  pdata->ignore_id_check = of_property_read_bool(np, "ftech,ignore-id-check");
  ...
  if (prop) {
    ...
    rc = of_property_read_u32_array(np, "ftech,button-map", button_map,
                                    num_buttons);
    if (rc) {
...
static ssize_t ctp_lockdown_proc_read(struct file *file, char __user *buf,
  ...
  *ppos += count;
  ...
  return sprintf(ptr, "%02X%02X%02X%02X%02X%02X%02X%02X\n", lockdown_info[0],
                 lockdown_info[1], lockdown_info[2], lockdown_info[3],
...
static void create_ctp_proc(void) {
  ...
  ctp_open_proc = proc_create(CTP_OPEN_PROC_NAME, 0777, ctp_device_proc,
                              &ctp_open_procs_fops);
  if (ctp_open_proc == NULL) {
    CTP_ERROR("ft5x06: create open_proc fail\n");
  }

  ctp_lockdown_proc = proc_create(CTP_LOCKDOWN_INFOR_NAME, 0666,
                                  ctp_device_proc, &ctp_lockdown_procs_fops);
  if (ctp_lockdown_proc == NULL) {
    CTP_ERROR("ft5x06: create ctp_lockdown_proc fail\n");
  }

  ctp_color_proc = proc_create(CTP_COLOR_INFOR_NAME, 0666, ctp_device_proc,
                               &ctp_color_procs_fops);
...
static ssize_t ft5x0x_debug_read(struct file *file, char __user *user_buf,
  ...
  case PROC_UPGRADE:
    ...
    if (ret < 0)
      num_read_chars =
          sprintf(debug_read_buf, "%s", "get fw version failed.\n");
    else
      num_read_chars =
          sprintf(debug_read_buf, "current fw version:0x%02x\n", regvalue);
    break;
  case PROC_READ_REGISTER:
    readlen = 1;
...
static int ft5x06_ts_probe(struct i2c_client *client,
  ...
  if (client->dev.of_node) {
    pdata = devm_kzalloc(&client->dev, sizeof(struct ft5x06_ts_platform_data),
                         GFP_KERNEL);
    if (!pdata) {
  ...
  data->early_suspend.level =
      EARLY_SUSPEND_LEVEL_BLANK_SCREEN + FT_SUSPEND_LEVEL;
...
static void __exit ft5x06_ts_exit(void) { i2c_del_driver(&ft5x06_ts_driver); }
